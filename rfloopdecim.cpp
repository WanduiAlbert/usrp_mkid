...
void recv_to_file(
	...
	int iper=0;
	int naccum=0;
	std::vector<sc32> accum(period,0);
    ...
    while(not stop_signal_called and ((num_requested_samples > num_total_samps) or (num_requested_samples == 0))){
		...
		for(size_t i=0;i<num_rx_samps;i++) {
			...
			if(iper == period) {
				...
				if(naccum == navg) {
					naccum = 0;
					vector<sc16> *v = new vector<sc16>(period);
...
int UHD_SAFE_MAIN(int argc, char *argv[]){
    ...
    desc.add_options()
		...
		("navg", po::value<int>(&navg)->default_value(1), "number of loops to average for decimation")
		("scaler", po::value<int>(&scaler)->default_value(1), "divide output by scaler before writing to disk")
	...
	int period = transmit_signal.size();
    ...
    //transmit_thread.create_thread(boost::bind(&transmit_worker, tx_stream, md, num_channels, transmit_signal, &stop_signal_called));
    transmit_thread.create_thread(boost::bind(&transmit_worker, tx_stream, num_channels, transmit_signal, &stop_signal_called, settling, num_request+1e6));

	double loop_period = transmit_signal.size() / tx_rate;
	cout << "loop period: " << loop_period << endl;
	recv_to_file(rx_usrp, spb, num_request, period, navg, scaler, rx_channel_nums, rx_stream, settling, output_file);
