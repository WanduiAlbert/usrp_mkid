...
void recv_to_file(
	...
	int iper=0;
	int naccum=0;
	std::vector<sc16> accum(period,0);
    ...
    while(not stop_signal_called and ((num_requested_samples > num_total_samps) or (num_requested_samples == 0))){
		...
		for(size_t i=0;i<num_rx_samps;i++) {
			...
			if(iper == period) {
				...
				if(naccum == navg) {
					naccum = 0;
					fwrite(&accum[0],period,sizeof(sc16),fout);
					memset(&accum[0],0,period*sizeof(sc16));
				}
			}
		}
...
int UHD_SAFE_MAIN(int argc, char *argv[]){
	...
	cout << "Waiting for output file to open...";
	int perms = S_IRUSR|S_IWUSR|S_IRGRP|S_IROTH;
	int fd = open(outfn.c_str(),O_NONBLOCK|O_WRONLY|O_TRUNC|O_CREAT,perms);
	...
	recv_to_file(rx_usrp, spb, num_request, period, navg, rx_channel_nums, rx_stream, settling, output_file);
