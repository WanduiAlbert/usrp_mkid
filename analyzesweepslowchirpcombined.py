...
def main():
	...
	# Average down the spectra to obtain a single representative asd
	repasd = np.mean(asds, axis=0)
	reppower = 1e18 * np.mean(asds/gains[:, np.newaxis], axis=0)
	dataset = np.vstack([fs, repasd, reppower]).T
	print dataset.shape
	np.savetxt("publication.txt", dataset, fmt="%10.5f",\
	delimiter=" ", header="Frequency	NEF		NEP")

	pl.figure(400)
	pl.gca().set_yscale('log')
	pl.gca().set_xscale('log')
	pl.grid()
	pl.plot(fs, repasd, label='%.1f'%txgains[0])
	pl.legend(loc='best')
	pl.xlabel('Frequency (Hz)')
	pl.ylabel('NEF (Hz/rtHz)')
	pl.title('NEF\nPheater = %.2fpW'%p0)
	pl.savefig('fig/averaged_asd.png')

	pl.figure(401)
	pl.gca().set_yscale('log')
	pl.gca().set_xscale('log')
	pl.grid()
	pl.plot(fs, repasd**2, label='%.1f'%txgains[0])
	pl.legend(loc='best')
	pl.xlabel('Frequency (Hz)')
	pl.ylabel('NEF^2 (Hz^2/Hz)')
	pl.title('NEF^2\nPheater = %.2fpW'%p0)
	pl.savefig('fig/averaged_psd.png')

	pl.figure(500)
	pl.gca().set_yscale('log')
	pl.gca().set_xscale('log')
	pl.grid()
	pl.plot(fs, reppower, label='%.1f'%txgains[0])
	pl.legend(loc='best')
	pl.xlabel('Frequency (Hz)')
	pl.ylabel('NEP (aW/rtHz)')
	pl.title('NEP\nPheater = %.2fpW'%p0)
	pl.savefig('fig/averaged_nep.png')
	pl.show()
