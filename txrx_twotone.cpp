...
void make_filename(std::string &out)
{
	char outstr[200];
	time_t t;
	struct tm *tmp;
	t = time(NULL);
	tmp = localtime(&t);
	strftime(outstr,sizeof(outstr),"%Y%m%d_%H%M%S.dat",tmp);
...
void transmit_worker(
    ...
    while(not stop_signal_called){
        ...
        for (size_t n = 0; n < buff.size(); n++){
            buff[n] = ampl1*nco(index1) + ampl2*nco(index2);
			index1 = (index1 + step1) & phase_mask;
			index2 = (index2 + step2) & phase_mask;
        }
...
template<typename samp_type> void recv_to_file(
    ...
    for (size_t i = 0; i < buffs.size(); i++) {
        ...
        outfiles.push_back(boost::shared_ptr<std::ofstream>(new std::ofstream(this_filename.c_str(), std::ofstream::binary|std::ofstream::app)));
    }
...
int UHD_SAFE_MAIN(int argc, char *argv[]){
    ...
    desc.add_options()
        ...
        ("settling", po::value<float>(&settling)->default_value(float(0.2)), "settling time (seconds) before receiving while sweeping TX DSP frequency in place")
        ("spb", po::value<size_t>(&spb)->default_value(0), "samples per buffer, 0 for default")
        ("rate", po::value<double>(&rate), "rate of samples")
        ("freq", po::value<double>(&freq), "RF center frequency in Hz")
        ("tone1", po::value<double>(&tone1), "Carrier1 frequency in Hz")
        ("tone2", po::value<double>(&tone2), "Carrier2 frequency in Hz")
		("lo-off", po::value<double>(&lo_off)->default_value(10e6), "LO offset frequency")
        ("ampl1", po::value<float>(&ampl1)->default_value(float(0.3)), "amplitude of the waveform [0 to 0.7]")
        ("ampl2", po::value<float>(&ampl2)->default_value(float(0.3)), "amplitude of the waveform [0 to 0.7]")
    ...
    for(size_t ch = 0; ch < tx_channel_nums.size(); ch++) {
        ...
        std::cout << boost::format("Setting TX Freq: %f MHz...") % ((freq+lo_off)/1e6) << std::endl;
        uhd::tune_request_t tx_tune_request(freq+lo_off);
    ...
    for(size_t ch = 0; ch < rx_channel_nums.size(); ch++) {
        ...
        std::cout << boost::format("Setting RX Freq: %f MHz...") % ((freq+lo_off)/1e6) << std::endl;
        uhd::tune_request_t rx_tune_request(freq+lo_off);
    ...
    unsigned int step1 = (unsigned int)round((tone1 - freq)*4294967296.0 / rate);
    unsigned int step2 = (unsigned int)round((tone2 - freq)*4294967296.0 / rate);
    ...
    transmit_thread.create_thread(boost::bind(&transmit_worker, buff, nco, tx_stream, md, step1, step2, ampl1, ampl2, num_channels));

	std::string file;
	make_filename(file);	

	FILE *fout = fopen(file.c_str(),"wb");
	unsigned int ntone = 2;
	fwrite(&ntone,sizeof(unsigned int),1,fout);
	fwrite(&ampl1,sizeof(unsigned int),1,fout);
	fwrite(&step1,sizeof(unsigned int),1,fout);
	fwrite(&ampl2,sizeof(unsigned int),1,fout);
	fwrite(&step2,sizeof(unsigned int),1,fout);
	fclose(fout);
