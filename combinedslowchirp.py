...
chirp_rate = 1.0e6 / 1024

gaincal_chopfreq = 5.0
gaincal_chopamp = 0.02 #0.04
gaincal_rb = 0.854e6
gaincal_rh = 0.106
...
def fndisk_name(fns):
	pre = fns[0].split('_')[0]
	start = fns[0].split('_')[1][:6]
	end = fns[-1].split('_')[1][:6]
	fntype = fns[-1].split('_')[1][6:]
	return pre + '_' + start + '_to_' + end + fntype +  '_freqts.npy'

def reduce_slowchirp(fns):
	fndisk = fndisk_name(fns)
	...
	allratios = []
	for fn in fns:
		allratios.append(loadsinglefile(fn, ns))
	
	ratio = np.concatenate(allratios)
	nt = ratio.size
	print nt
	#nt2 = nchunk * ns
	#ratio = ratio[nt-nt2:]
...
def analyze_noise(dates):
	fns = [os.path.join('raw',date+'gaincal') for date in dates]
	y = reduce_slowchirp(fns)
...
def analyze_gain(dates, vdcs):
	fns = [os.path.join('raw',date+'gaincal') for date in dates]
	y = reduce_slowchirp(fns)
	gain = chirpanal.gaincal(y,vdcs[0],gaincal_chopamp,gaincal_chopfreq,gaincal_rh,gaincal_rb,chirp_rate)
...
def rawreduce(fn):
	...
	# We want to combine all the files before the analysis
	gain = analyze_gain(dates, vdcs) # All the files should have the same dc offset
	fs, asd = analyze_noise(dates)
	...
	d = {}
	d['dates'] = dates
	d['temps'] = np.array(temps)[0]
	d['txgains'] = np.array(txgains)[0]
	d['vdcs'] = np.array(vdcs)[0]
	d['asds'] = asd 
	d['gains'] = gain
	d['fs'] = np.array(fs)
...
def main():
	...
	#pl.scatter(txgains[ok],meannepread[ok],marker='x',label='380-400Hz pheat=%.2fpW'%p0,color=colors[j])
	#pl.xlabel('TXGain (dB)')
	#pl.ylabel('NEP (aW/rtHz)')
	#pl.grid()
	#pl.ylim(ymin=0)
